...
//《改》
inline void torsions_randomize0(flv& torsions, rng& generator) {
	VINA_FOR_IN(i, torsions)
		torsions[i] = 0;
}
...
struct ligand_conf {
	...
	//《改》
		void randomize0(const vec& corner1, const vec& corner2, rng& generator) {
		rigid.randomize(corner1, corner2, generator);
		torsions_randomize0(torsions, generator);
	}
	void print() const {
		rigid.print();
		printnl(torsions);
...
struct conf {
	...
	//《改》
		void randomize0(const vec& corner1, const vec& corner2, rng& generator) {
		VINA_FOR_IN(i, ligands)
			ligands[i].randomize0(corner1, corner2, generator);
		VINA_FOR_IN(i, flex)
			flex[i].randomize(generator);
	}
	void print() const {
...
//《改》
inline bool operator<(const sort_type& a, const sort_type& b) { // for sorting Esort
	return a.e < b.e;
}
#endif
