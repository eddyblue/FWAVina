...
const atom_kind atom_kind_data[] = { // name, radius, depth, solvation parameter, volume, covalent radius
	...
	{"Ru",    1.48000,    0.05600,   -0.00110,   12.00000,   1.25}, // 15
...
//{"Ru",    1.48000,    0.05600,   -0.00110,   12.00000,   1.25}
//将Zn替换成了Ru，参数在这里设置。
};
...
inline bool ad_is_hydrogen(sz ad) {                            //判断是否是氢原子
	return ad == AD_TYPE_H || ad == AD_TYPE_HD;
}

inline bool ad_is_heteroatom(sz ad) { // returns false for ad >= AD_TYPE_SIZE    //判断是否是杂原子
	return ad != AD_TYPE_A && ad != AD_TYPE_C  && 
		   ad != AD_TYPE_H && ad != AD_TYPE_HD && 
		   ad < AD_TYPE_SIZE;
}

inline sz ad_type_to_el_type(sz t) {                               //将AD类型转为EL类型
	switch(t) {
