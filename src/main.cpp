...
#include "parallel_fwa.h" //FWAVina
#include "file.h"
#include "cache.h"
#include "non_cache.h"
...
void do_search(model& m, const boost::optional<model>& ref, const scoring_function& sf, const precalculate& prec, const igrid& ig, const precalculate& prec_widened, const igrid& ig_widened, non_cache& nc, // nc.slope is changed
			   ...
			   const parallel_fwa& par/*FWAVina*/, fl energy_range, sz num_modes, 
			   int seed, int verbosity, bool score_only, bool local_only, tee& log, const terms& t, const flv& weights,int num_fwa, double w, double c1, double c2) {
	...
	if(score_only) {
		...
		refine_structure(m, prec, nc, out, authentic_v, par.fwa.ssd_par.evals/*FWAVina*/);
		done(verbosity, log);
		...
		par(m, out_cont, prec, ig, prec_widened, ig_widened, corner1, corner2, generator, num_fwa, w, c1, c2);
		done(verbosity, log);
...
if(!out_cont.empty()) {
		...
		VINA_FOR_IN(i, out_cont)
			refine_structure(m, prec, nc, out_cont[i], authentic_v, par.fwa.ssd_par.evals/*FWAVina*/);
...
void main_procedure(model& m, const boost::optional<model>& ref, // m is non-const (FIXME?)
				 ...
				 const flv& weights,
				 int cpu, int seed, int verbosity, sz num_modes, fl energy_range, tee& log, int num_fwa,double w,double c1,double c2) {
	...
	parallel_fwa par;/*FWAVina*/
	sz heuristic = m.num_movable_atoms() + 10 * m.get_size().num_degrees_of_freedom();
	par.fwa.num_steps = unsigned(70 * 3 * (50 + heuristic) / 2); // 2 * 70 -> 8 * 20 // FIXME/*FWAVina*/
	par.fwa.ssd_par.evals = unsigned((25 + m.num_movable_atoms()) / 3);/*FWAVina*/
	par.fwa.min_rmsd = 1.0;/*FWAVina*/
	par.fwa.num_saved_mins = 20;/*FWAVina*/
	par.fwa.hunt_cap = vec(10, 10, 10);/*FWAVina*/
	par.num_tasks = exhaustiveness;
	par.num_threads = cpu;
	par.display_progress = (verbosity > 1);
//输出迭代次数	
FILE *fplj;
int iter=par.fwa.num_steps ; 
	...
	if(randomize_only) {
		...
		if(no_cache) {
			do_search(m, ref, wt, prec, nc, prec_widened, nc_widened, nc,
					  ...
					  seed, verbosity, score_only, local_only, log, t, weights,num_fwa,w,c1,c2);
		}
		else {
...
int main(int argc, char* argv[]) {
	...
	const std::string version_string = "FWAVina version 1.0";    //fwa
...
=======================================================================\n\
FWAVina was developed based on the framework of AutoDock Vina.         \n\
                                                                       \n\
...
=======================================================================\n";
	...
	try {
		...
		int cpu = 0, seed, exhaustiveness, verbosity = 2, num_modes =10,num_fwa = 0;
		fl energy_range = 2.0;
		...
		;
		options_description fwa("FWA parameters (optional)");
		fwa.add_options()
			("num_firework", value<int>(&num_fwa)->default_value(9), "Number of firework per thread")
		...
		desc       .add(inputs).add(search_area).add(fwa).add(outputs).add(advanced).add(misc).add(config).add(info);
		desc_config.add(inputs).add(search_area).add(fwa).add(outputs).add(advanced).add(misc);
		desc_simple.add(inputs).add(search_area).add(fwa).add(outputs).add(misc).add(config).add(info);
		...
		main_procedure(m, ref, 
					...
					gd, exhaustiveness,
					weights,
					cpu, seed, verbosity, max_modes_sz, energy_range, log, num_fwa,w,c1,c2);
	}
	catch(file_error& e) {
